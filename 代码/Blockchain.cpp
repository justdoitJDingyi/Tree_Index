...
// Block type enumeration (8KB/16KB/32KB/64KB) 块类型枚举
enum BlockType {
...
// Block size in bytes for each block type （定义每种块类型对应的实际字节大小）
...
// Records per block for each block type（每种块类型中包含的记录数量）
...
struct FileLocation {
    BlockType block_type;  // Block type 文件元数据所在的块类型
    uint32_t chunk_id;     // Chunk ID   表示该块在块池中的编号
    uint32_t slot;         // Slot offset within the chunk  表示该块中的槽位偏移，即slot=5 就表示在块的第 6 个槽
...
// Chunk node (linked list structure) 描述一个物理块在链表里的信息
// 
struct ChunkNode {
